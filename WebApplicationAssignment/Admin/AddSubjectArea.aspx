...
<asp:Content ID="Content1" ContentPlaceHolderID="cpMainSection" Runat="Server"><!-- start of content -->
    ...
    <p>This page you can import an image ready to be used to be uploaded so the user can see it and can also write some information for that subject area in the website too.</p>
        ...
        <asp:Label ID="Lbl_DatabaseConnection" runat="server" CssClass="errorMessage" Visible="false" Text="Cannot connect to the database." />
         <div class="addData"><!--start of a div with the class addData -->
        <asp:FileUpload ID="FileUpload1" runat="server" /><!-- feature to upload images -->

             <!-- upload button to upload images into the database -->
        <asp:Button ID="btnUpload" runat="server" Text="Upload" OnClick="BtnUpload_Click" CausesValidation="false" SkinID="Button" />
        <asp:Label ID="lblMessage" runat="server" Text="" Font-Names = "Arial"></asp:Label><!-- label for messages -->
         </div><!-- end of div -->
    ...
    <asp:DetailsView ID="addSubjectArea" runat="server" CssClass="addData" AutoGenerateRows="False" DataKeyNames="ID" DataSourceID="AddIntoSubjectAreaTable" DefaultMode="Insert" 
     ...
     <Fields>
      ...
      <asp:TemplateField HeaderText="Name" SortExpression="Name_SA">
        <EditItemTemplate>
          <asp:TextBox ID="Txt_SA_Name" runat="server" CssClass="txt_addData" Text='<%# Bind("Name_SA") %>'></asp:TextBox><br />
      ...
      <asp:TemplateField HeaderText="Extra Information" SortExpression="Extra Information">
        <EditItemTemplate>
          <asp:TextBox ID="Txt_Extra_Info_SA" CssClass="txt_addData" TextMode="MultiLine" Width="300" Height="100" runat="server" Text='<%# Bind("AdditionalInformation") %>'></asp:TextBox>
      ...
      <asp:TemplateField HeaderText="Image" SortExpression="ImageID">
        <EditItemTemplate>
          <asp:DropDownList ID="DropDownList1" runat="server" CssClass="txt_addData" DataSourceID="ImageDataSource" DataTextField="Name" DataValueField="ID" 
              SelectedValue='<%# Bind("ImageID") %>'>
          </asp:DropDownList>
        </EditItemTemplate>
        <InsertItemTemplate>
          <asp:DropDownList ID="DropDownList1" runat="server" CssClass="txt_addData" DataSourceID="ImageDataSource" DataTextField="Name" DataValueField="ID" 
    ...
    <asp:SqlDataSource ID="AddIntoSubjectAreaTable" runat="server" ConnectionString="<%$ ConnectionStrings:HigherEducationChesterfieldCollegeConnectionString %>" 
        ...
        InsertCommand="INSERT INTO [SubjectArea] ([Name_SA], [ImageID], [AdditionalInformation]) VALUES (@Name_SA, @ImageID, @AdditionalInformation)" 
        SelectCommand="SELECT * FROM [SubjectArea]" UpdateCommand="UPDATE [SubjectArea] SET [Name_SA] = @Name_SA, [ImageID] = @ImageID, 
        [AdditionalInformation] = @AdditionalInformation WHERE [ID] = @ID">
        <DeleteParameters>
            <asp:Parameter Name="ID" Type="Int32" />
        </DeleteParameters>
        <InsertParameters>
            <asp:Parameter Name="Name_SA" Type="String" />
    ...
    <asp:SqlDataSource ID="AddIntoImageTable" runat="server" ConnectionString="<%$ ConnectionStrings:HigherEducationChesterfieldCollegeConnectionString %>"
        DeleteCommand="DELETE FROM [Image] WHERE [ID] = @ID" InsertCommand="INSERT INTO [Image] ([Name], [ContentType], [Data]) VALUES (@Name, @ContentType, @Data)" 
        SelectCommand="SELECT * FROM [Image]" UpdateCommand="UPDATE [Image] SET [Name] = @Name, [ContentType] = @ContentType, [Data] = @Data WHERE [ID] = @ID">
        <DeleteParameters>
            <asp:Parameter Name="ID" Type="Int32" />
        </DeleteParameters>
        <InsertParameters>
