...
public partial class MasterPages_Master_User : System.Web.UI.MasterPage /* the masterpages_Master_User page is inherited to the System.Web.UI.MasterPage */
{
    ...
    protected void Page_Load(object sender, EventArgs e) /* start of page_load method */
    {
        ...
        if (!Page.IsPostBack) /* if the page does not postback */
        {
            ...
            con.Open();
            SqlCommand cmd = new SqlCommand("SELECT * FROM [UserLogin] WHERE [LoggedOn]=@LoggedOn", con);
            cmd.Parameters.AddWithValue("@LoggedOn", 1);
    ...
    protected void Btn_LogOut_Click(object sender, EventArgs e) /*when the login button is pressed */
    {
        ...
        /*updating value*/
        SqlCommand cmd = new SqlCommand("UPDATE [UserLogin] SET [LoggedOn]=@LoggedOn", con);
        cmd.Parameters.AddWithValue("@LoggedOn", false);

        /*checking the visible state of buttons */
        Btn_LogIn.Visible = true;
        Btn_SignUp.Visible = true;
        Btn_LogOut.Visible = false;
